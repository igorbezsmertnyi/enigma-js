...
const getKeyCode = () => {
  let = TxKey1 = document.getElementById('Key1').value
  let = TxKey2 = document.getElementById('Key2').value
  let = TxKey3 = document.getElementById('Key3').value
  let KeyCode = document.getElementById('keyCode')
  KeyCode.innerHTML = `Key: ${TxKey1} ${TxKey2} ${TxKey3}`
}
...
//general form processing function. Reading keyboard event
const form_encrypt = (event) => {
  if(event.keyCode >= 65 && event.keyCode <= 90) {          //selectiong of values more than 65 and less thsn 90
    ...
    TxKey3 = document.getElementById('Key3').value
    let TxArea = document.getElementById('shiprograms')

    if(KeyboardEvent) {                                     //changing key box values
      n = (n + 1) % 26
      k++
      TxKey3 = arr_alp[(find(arr_alp, TxKey3) + 1) % 26]

      if(n == 3 || n == 7 || n == 13 || n == 17 || n == 21 || n == 23) {
        TxKey2 = arr_alp[(find(arr_alp, TxKey2) + 1) % 26]
      }
      if(n == 5 || n == 10 || n == 15 || n == 20 || n == 25) {
        TxKey1 = arr_alp[(find(arr_alp, TxKey1) + 1) % 26]
      }
    }

    document.getElementById('Key3').value = arr_alp[(find(arr_alp, TxKey3) + n1) % 26]
    document.getElementById('Key2').value = arr_alp[(find(arr_alp, TxKey2) + n2) % 26]
    document.getElementById('Key1').value = arr_alp[(find(arr_alp, TxKey1) + n3) % 26]

    r_1 = (find(arr_alp, TxEncrypt) + find(arr_alp, TxKey3)) % 26
    r_2 = (find(arr_alp, rotor_1[r_1]) + Module(find(arr_alp, TxKey2), find(arr_alp, TxKey3))) % 26
    r_3 = (find(arr_alp, rotor_2[r_2]) + Module(find(arr_alp, TxKey1), find(arr_alp, TxKey2))) % 26
    refl = (Module(find(arr_alp, rotor_3[r_3]), find(arr_alp, TxKey1))) % 26
    r_4 = (find(arr_alp, reflector[refl]) + find(arr_alp, TxKey1)) % 26
    r_5 = (Module(find(rotor_3, arr_alp[r_4]), Module(find(arr_alp, TxKey1), find(arr_alp, TxKey2)))) % 26
    r_6 = (Module(find(rotor_2, arr_alp[r_5]), Module(find(arr_alp, TxKey2), find(arr_alp, TxKey3)))) % 26
    Encrypt = (Module(find(rotor_1, arr_alp[r_6]), find(arr_alp, TxKey3))) % 26
